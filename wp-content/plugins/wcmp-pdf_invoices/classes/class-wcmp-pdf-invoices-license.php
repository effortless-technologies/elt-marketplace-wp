<?php
if (get_option('wcmp_pdf_invoices_license_activated') != 'Activated') {
    add_action('admin_notices', 'WCMp_PDF_Invoices_License::license_inactive_notice');
}
...
class WCMp_PDF_Invoices_License {
    ...
    public $upgrade_url = WCMp_PDF_INVOICES_PLUGIN_SERVER_URL; // URL to access the Update API Manager.

    /**
     * @var string
     * This version is saved after an upgrade to compare this db version to $version
     */
    public $api_manager_license_version_name = 'wcmp-pdf-invoices_license_version';
    ...
    public $license_update_check = 'wcmp_pdf_invoices_update_check';
    ...
    public function __construct() {
        ...
        if (is_admin()) {
             ...
             */
            $this->license_software_product_id = $WCMp_PDF_Invoices->token;

            /**
             * Set all data defaults here
             */
            $this->license_data_key = 'wcmp_' . str_replace('-', '_', esc_attr($WCMp_PDF_Invoices->token)) . '_license_settings_name';
            $this->license_api_key = 'api_key';
            $this->license_activation_email = 'activation_email';
            $this->license_product_id_key = 'wcmp_pdf_invoices_license_product_id';
            $this->license_instance_key = 'wcmp_pdf_invoices_license_instance';
            $this->license_deactivate_checkbox_key = 'wcmp_pdf_invoices_license_deactivate_checkbox';
            $this->license_activated_key = 'wcmp_pdf_invoices_license_activated';
            ...
            $this->license_plugin_name = 'wcmp-pdf_invoices/wcmp_pdf_invoices.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
            $this->license_product_id = get_option($this->license_product_id_key); // Software Title
            $this->license_renew_license_url = WCMp_PDF_INVOICES_PLUGIN_SERVER_URL . '/my-account'; // URL to renew a license
            ...
            $this->license_software_version = $WCMp_PDF_Invoices->version; // The software version
            ...
            $this->api_manager_license_key = new WCMp_PDF_Invoices_Key_Api();
    ...
    public function add_menu() {
        global $WCMp, $WCMp_PDF_Invoices, $GLOBALS;

        if (empty($GLOBALS['admin_page_hooks']['wcmp_licenses'])) {
            if (!class_exists('WCMp_License')) {
                require_once ($WCMp_PDF_Invoices->plugin_path . 'classes/license/admin/class-wcmp-license.php');
                new WCMp_License('wcmp_pdf_invoices_license');
            }
        }

        add_filter('wcmp_license_tabs', array(&$this, 'license_new_tab'), 10, 1);
        add_action('settings_page_' . str_replace('-', '_', esc_attr($WCMp_PDF_Invoices->token)) . '_license_tab_init', array(&$this, 'license_tab_init'), 10, 1);
    }

    function license_new_tab($tabs) {
        global $WCMp_PDF_Invoices;
        $tabs[str_replace('-', '_', esc_attr($WCMp_PDF_Invoices->token)) . '_license'] = __('PDF Invoices', 'wcmp-pdf_invoices'
);
        return $tabs;
    }
    ...
    function license_tab_init($tab) {
        ...
        new WCMp_PDF_Invoices_Settings_License($tab);
    }

    function load_class($class_name = '') {
        global $WCMp_PDF_Invoices;
        if ('' != $class_name) {
            require_once ($WCMp_PDF_Invoices->plugin_path . '/classes/license/classes/class-' . esc_attr($WCMp_PDF_Invoices->token) . '-' . esc_attr($class_name) . '.php');
        } // End If Statement
    }
    ...
    function load_class_admin($class_name = '') {
        global $WCMp_PDF_Invoices;
        if ('' != $class_name) {
            require_once ($WCMp_PDF_Invoices->plugin_path . '/classes/license/admin/class-' . esc_attr($WCMp_PDF_Invoices->token) . '-' . esc_attr($class_name) . '.php');
        } // End If Statement
    }
    ...
    public function activation() {
        ...
        $WCMp_PDF_Invoices_API_Manager_Password = new WCMp_PDF_Invoices_API_Manager_Password();
        $instance = $WCMp_PDF_Invoices_API_Manager_Password->generate_password(12, false);
        ...
        if (version_compare($this->license_software_version, $curr_ver, '>')) {
            // update the version
            update_option($this->api_manager_license_version_name, $WCMp_PDF_Invoices->version);
        }
    ...
    public static function license_inactive_notice() {
        ...
        <div id="message" class="error settings-error notice is-dismissible">
            <p><?php printf(__('The PDF Invoices License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'wcmp-pdf_invoices'
), '<a href="' . esc_url(admin_url('admin.php?page=wcmp-license&tab=' . str_replace('-', '_', esc_attr($WCMp_PDF_Invoices->token)) . '_license')) . '">', '</a>'); ?></p>
        </div>
        <?php
    }
...
function WCMp_PDF_Invoices_LICENSE() {
    return WCMp_PDF_Invoices_License::instance();
}
