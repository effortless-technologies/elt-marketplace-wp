        ...
        protected function get_products( $pms=array() ) {
            ...
            foreach ($res as $id => $val) {
                ...
                if ( isset($parent2child["$id"], $parent2child["$id"]->nb)
                    ...
                    $childs_btn = '<a href="#" class="wz-show-variations' . ($is_open ? ' sign-minus' : ' sign-plus') . '">(<span>' . ($is_open ? '<i class="fa fa-caret-up"></i>' : '<i class="fa fa-caret-down"></i>') . '</span>' . $childs_nb . ')</a>';
                    $nbprodv += $childs_nb;
