...
<div class="WooZone-cart-checkout">
	...
	<div class="WooZone-cart-msg"></div>
		...
		public function ajax_requests()
		{
			...
			$allowed_action = array( 'save_countries', 'save_product_country', 'load_cross_sell', 'cross_sell_empty_cache', 'load_amazon_reviews' );
				...
				if ( $url != "" ) {
					...
					if ( trim($params['exp']) != "" ) {
						...
						if ( time() > $expire_on ) {
							...
							$post_id = $prodid;
							if( $post_id > 0 ){
								$new_url = $this->the_plugin->amzHelper->updateProductReviews( $post_id );
