jQuery(document).ready(function($) {
	$('.wcmp_verification .do_verification').on('click', function(e){
		e.preventDefault();
		var $this = $(this);
		var verification_type = $(this).attr('data-verification');
		var user_id = $(this).attr('data-user_id');
		var data_action = $(this).attr('data-action');
		var data = {
			action   : 'wcmp_vendor_verification_approval',
		...
		$.post(admin_ajax.ajax_url, data, function(response) {
			...
			location.reload();
			var data2 = {
			action   : 'wcmp_vendor_is_verified_approval',
			user_id : user_id
			}
			$.post(admin_ajax.ajax_url, data2, function(response) {
			});
		});
	});
