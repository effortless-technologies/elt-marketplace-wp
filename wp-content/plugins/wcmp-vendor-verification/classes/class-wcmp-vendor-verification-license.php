<?php
if (get_option('wcmp_vendor_verification_license_activated') != 'Activated') {
    add_action('admin_notices', 'WCMP_Vendor_Verification_License::license_inactive_notice');
}

class WCMP_Vendor_Verification_License {

    /**
     * Self Upgrade Values
     */
    // Base URL to the remote upgrade API server
    public $upgrade_url = WCMP_VENDOR_VERIFICATION_PLUGIN_SERVER_URL; // URL to access the Update API Manager.

    /**
     * @var string
     * This version is saved after an upgrade to compare this db version to $version
     */
    public $api_manager_license_version_name = 'wcmp-vendor-verification_license_version';
    ...
    public $license_update_check = 'wcmp_vendor_verification_update_check';
    ...
    public function __construct() {
        ...
        if (is_admin()) {
             ...
             */
            $this->license_software_product_id = $WCMP_Vendor_Verification->token;

            /**
             * Set all data defaults here
             */
            $this->license_data_key = 'wcmp_' . str_replace('-', '_', esc_attr($WCMP_Vendor_Verification->token)) . '_license_settings_name';
            $this->license_api_key = 'api_key';
            $this->license_activation_email = 'activation_email';
            $this->license_product_id_key = 'wcmp_vendor_verification_license_product_id';
            $this->license_instance_key = 'wcmp_vendor_verification_license_instance';
            $this->license_deactivate_checkbox_key = 'wcmp_vendor_verification_license_deactivate_checkbox';
            $this->license_activated_key = 'wcmp_vendor_verification_license_activated';
            ...
            $this->api_manager_license_key = new WCMP_Vendor_Verification_Key_Api();
    ...
    public function add_menu() {
        global $WCMp, $WCMP_Vendor_Verification, $GLOBALS;

        if (empty($GLOBALS['admin_page_hooks']['wcmp_licenses'])) {
            if (!class_exists('WCMp_License')) {
                require_once ($WCMP_Vendor_Verification->plugin_path . 'classes/license/admin/class-wcmp-license.php');
                new WCMp_License('wcmp_vendor_verification_license');
            }
        }

        add_filter('wcmp_license_tabs', array(&$this, 'license_new_tab'), 10, 1);
        add_action('settings_page_' . str_replace('-', '_', esc_attr($WCMP_Vendor_Verification->token)) . '_license_tab_init', array(&$this, 'license_tab_init'), 10, 1);
        
        /*global $WCMP_Vendor_Verification;
        add_filter('WCMP_Vendor_Verification_tabs', array(&$this, 'license_new_tab'), 10, 1);
        add_action('settings_page_' . str_replace('-', '_', esc_attr($WCMP_Vendor_Verification->token)) . '_license_tab_init', array(&$this, 'license_tab_init'), 10, 1);*/
    }

    function license_new_tab($tabs) {
        global $WCMP_Vendor_Verification;
        $tabs[str_replace('-', '_', esc_attr($WCMP_Vendor_Verification->token)) . '_license'] = __('Vendor Verification', 'wcmp-vendor-verification');
        return $tabs;
    }

    function license_tab_init($tab) {
        $this->load_class_admin("settings-license");
        new WCMP_Vendor_Verification_Settings_License($tab);
    }

    function load_class($class_name = '') {
        global $WCMP_Vendor_Verification;
        if ('' != $class_name) {
            require_once ($WCMP_Vendor_Verification->plugin_path . '/classes/license/classes/class-' . esc_attr($WCMP_Vendor_Verification->token) . '-' . esc_attr($class_name) . '.php');
        } // End If Statement
    }
...
// End load_class()
    ...
    function load_class_admin($class_name = '') {
        global $WCMP_Vendor_Verification;
        if ('' != $class_name) {
            require_once ($WCMP_Vendor_Verification->plugin_path . '/classes/license/admin/class-' . esc_attr($WCMP_Vendor_Verification->token) . '-' . esc_attr($class_name) . '.php');
        } // End If Statement
    }
...
// End load_class()
    ...
    public function activation() {
        global $wpdb, $WCMP_Vendor_Verification;
        ...
        $WCMP_Vendor_Verification_API_Manager_Password = new WCMP_Vendor_Verification_API_Manager_Password();
        $instance = $WCMP_Vendor_Verification_API_Manager_Password->generate_password(12, false);
        ...
        if (version_compare($WCMP_Vendor_Verification->version, $curr_ver, '>')) {
            // update the version
            update_option($this->api_manager_license_version_name, $WCMP_Vendor_Verification->version);
        }
    ...
    public static function license_inactive_notice() {
        ...
        <div id="message" class="error">
            <p><?php printf(__('The Vendor Verification License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'wcmp-vendor-verification'), '<a href="' . esc_url(admin_url('admin.php?page=wcmp-license&tab=' . str_replace('-', '_', esc_attr($WCMP_Vendor_Verification->token)) . '_license')) . '">', '</a>'); ?></p>
        </div>
        <?php
    }
...
function WCMP_Vendor_Verification_LICENSE() {
    return WCMP_Vendor_Verification_License::instance();
}
