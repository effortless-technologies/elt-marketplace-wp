    ...
    public function save_vendor_verification_data() {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST' && $WCMp->endpoints->get_current_endpoint() == 'vendor-verification') { 
            if (isset($_POST['vendor_id_proof_sbmt'])) {
                ...
                if(isset($_POST['vendor_verify_type']) && !empty($_POST['vendor_verify_type'])){
                    ...
                    if(isset($_POST['vendor_verify_id_image']) && !empty($_POST['vendor_verify_id_image']))
                        $data['verification_file'] = esc_url($_POST['vendor_verify_id_image']);
            ...
            if (isset($_POST['vendor_addrs_proof_sbmt'])) {
                ...
                $data = array();
                if(isset($_POST['vendor_verification_address_1']) && !empty($_POST['vendor_verification_address_1']))
                    $data['address_1'] = sanitize_textarea_field($_POST['vendor_verification_address_1']);
                if(isset($_POST['vendor_verification_address_2']) && !empty($_POST['vendor_verification_address_2']))
                    $data['address_2'] = sanitize_textarea_field($_POST['vendor_verification_address_2']);
                if(isset($_POST['vendor_verification_country']) && !empty($_POST['vendor_verification_country']))
                    $data['country'] = sanitize_text_field($_POST['vendor_verification_country']);
                if(isset($_POST['vendor_verification_state']) && !empty($_POST['vendor_verification_state']))
                    $data['state'] = sanitize_text_field($_POST['vendor_verification_state']);
                if(isset($_POST['vendor_verification_city']) && !empty($_POST['vendor_verification_city']))
                    $data['city'] = sanitize_text_field($_POST['vendor_verification_city']);
                if(isset($_POST['vendor_verification_postcode']) && !empty($_POST['vendor_verification_postcode']))
                    $data['postcode'] = sanitize_text_field($_POST['vendor_verification_postcode']);
