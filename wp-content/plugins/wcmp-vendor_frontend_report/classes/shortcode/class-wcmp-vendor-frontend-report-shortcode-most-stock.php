<?php
class WCMP_Vendor_Report_Shortcode_Most_Stock {
		...
		if(function_exists( 'is_user_wcmp_vendor' ) ) {
			if(is_user_wcmp_vendor($user_id)) {
				...
				if(is_user_wcmp_vendor($user_id)) {
					...
					wp_localize_script('wcmp_frontend_report_low_stock_js', 'wcmp_frontend_report_most_stocked', array( 'max_items' => $max_items ));
					
					
					$more_button = true;
					if(count($items) == $max_items) $more_button = false;
					
					$WCMp_Vendor_Frontend_Report->template->get_template( 'reports/most_stocked.php' , array('items' => $items, 'max_items' => $max_items, 'current_page' => $current_page + 1, 'per_page' => $per_page, 'show_more_button' => $more_button) );
				}
			} else { ?>
					<div>
