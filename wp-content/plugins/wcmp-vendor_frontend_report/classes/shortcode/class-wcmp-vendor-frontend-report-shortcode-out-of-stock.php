<?php
class WCMP_Vendor_Report_Shortcode_Out_Of_Stock {
	...
	public static function output( $attr ) {
		...
		if( function_exists( 'is_user_wcmp_vendor' ) ) {
			if(is_user_wcmp_vendor($user_id)) {
				...
				//wp_enqueue_script('frontend_report_out_of_stock_js', $frontend_script_path.'out_of_stock.js', array('jquery'), $WCMp_Vendor_Frontend_Report->version, true);
				...
				if(is_user_wcmp_vendor($user_id)) {
					...
					if ( WCMp_Vendor_Frontend_Report_Dependencies::wcmp_vendor_stock_alert_plugin_active_check()) {
						if(!$stock) {
							...
							if(!empty($stock_alert_settings) && isset($stock_alert_settings['out_of_stock_limit']) && !empty($stock_alert_settings['out_of_stock_limit'])) $stock   = absint( $stock_alert_settings['out_of_stock_limit'] );
							if(!$stock) {
								$wcmp_stock_alert_settings = get_wcmp_vendor_settings( 'wcmp_product_settings_name' );
								if(!empty($wcmp_stock_alert_settings) && isset($wcmp_stock_alert_settings['out_of_stock_limit']) && !empty($wcmp_stock_alert_settings['out_of_stock_limit'])) $stock   = absint( $wcmp_stock_alert_settings['out_of_stock_limit'] );
							}
						}
					}
					
					if(!$stock) {
						$stock   = absint( max( get_option( 'woocommerce_notify_no_stock_amount' ), 1 ) );
					}
					...
					wp_localize_script('wcmp_frontend_report_low_stock_js', 'wcmp_frontend_report_out_of_stock', array( 'max_items' => $max_items ));
					
					$WCMp_Vendor_Frontend_Report->template->get_template( 'reports/out_of_stock.php' , array('items' => $items, 'max_items' => $max_items, 'current_page' => $current_page + 1, 'per_page' => $per_page, 'show_more_button' => $more_button) );
				}
			} else { ?>
				<div>
