...
if ( get_option( 'wcmp_vendor_frontend_report_license_activated' ) != 'Activated' ) {
  add_action( 'admin_notices', 'WCMP_Vendor_Frontend_Report_License::license_inactive_notice' );
}

class WCMP_Vendor_Frontend_Report_License {
	
	/**
	 * Self Upgrade Values
	 */
	// Base URL to the remote upgrade API server
	public $upgrade_url = WCMP_VENDOR_FRONTEND_REPORT_PLUGIN_SERVER_URL; // URL to access the Update API Manager.

	/**
	 * @var string
	 * This version is saved after an upgrade to compare this db version to $version
	 */
	public $api_manager_license_version_name = 'wcmp-vendor-frontend-report_license_version';
	...
	public $license_update_check = 'wcmp_vendor_frontend_report_update_check';
    ...
    global $WCMp_Vendor_Frontend_Report;
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->license_software_product_id = $WCMp_Vendor_Frontend_Report->token;

			/**
			 * Set all data defaults here
			 */
			$this->license_data_key 				= 'wcmp_' . str_replace('-', '_', esc_attr($WCMp_Vendor_Frontend_Report->token)) . '_license_settings_name';
			$this->license_api_key 					= 'api_key';
			$this->license_activation_email 		= 'activation_email';
			$this->license_product_id_key 			= 'wcmp_vendor_frontend_report_license_product_id';
			$this->license_instance_key 			= 'wcmp_vendor_frontend_report_license_instance';
			$this->license_deactivate_checkbox_key 	= 'wcmp_vendor_frontend_report_license_deactivate_checkbox';
			$this->license_activated_key 			= 'wcmp_vendor_frontend_report_license_activated';
			...
			$this->license_plugin_name 			= 'wcmp-vendor_frontend_report/wcmp_vendor_frontend_report.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
			$this->license_product_id 			= get_option( $this->license_product_id_key ); // Software Title
			$this->license_renew_license_url 	= WCMP_VENDOR_FRONTEND_REPORT_PLUGIN_SERVER_URL.'/my-account'; // URL to renew a license
			...
			$this->license_software_version 	= $WCMp_Vendor_Frontend_Report->version; // The software version
      ...
      $this->api_manager_license_key = new WCMP_Vendor_Frontend_Report_Key_Api();
      ...
      $this->load_class('update');
			...
			if ( ! empty( $options ) && $options !== false ) {
				...
				new WCMP_Vendor_Frontend_Report_API_Manager_Update_API_Check(
					$this->upgrade_url,
	...
	public function add_menu() {
	  global $WCMp, $WCMp_Vendor_Frontend_Report, $GLOBALS;
	  
	  if ( empty ( $GLOBALS['admin_page_hooks']['wcmp_licenses'] ) )  {
	  	if(!class_exists('WCMp_License')) {
	  		require_once ($WCMp_Vendor_Frontend_Report->plugin_path . 'classes/license/admin/class-wcmp-license.php');
	  		new WCMp_License('wcmp_vendor_frontend_report_license');
	  	}
		}
		
		add_filter('wcmp_license_tabs', array(&$this, 'license_new_tab'), 10, 1);
	  add_action( 'settings_page_' . str_replace('-', '_', esc_attr($WCMp_Vendor_Frontend_Report->token)) . '_license_tab_init', array(&$this, 'license_tab_init'), 10, 1);
	}
    ...
    new WCMP_Vendor_Frontend_Report_Settings_License($tab);
  }
	
	function load_class($class_name = '') {
	  global $WCMp_Vendor_Frontend_Report;
		if ('' != $class_name) {
			require_once ($WCMp_Vendor_Frontend_Report->plugin_path . '/classes/license/classes/class-' . esc_attr($WCMp_Vendor_Frontend_Report->token) . '-' . esc_attr($class_name) . '.php');
		} // End If Statement
	}// End load_class()
	...
	function load_class_admin($class_name = '') {
	  global $WCMp_Vendor_Frontend_Report;
		if ('' != $class_name) {
			require_once ($WCMp_Vendor_Frontend_Report->plugin_path . '/classes/license/admin/class-' . esc_attr($WCMp_Vendor_Frontend_Report->token) . '-' . esc_attr($class_name) . '.php');
		} // End If Statement
	}// End load_class()
	...
	public function activation() {
		global $wpdb, $WCMp_Vendor_Frontend_Report;
		...
		$WCMp_Vendor_Frontend_Report_API_Manager_Password = new WCMP_Vendor_Frontend_Report_API_Manager_Password();
		$instance = $WCMp_Vendor_Frontend_Report_API_Manager_Password->generate_password( 12, false );
		...
		if ( version_compare( $this->license_software_version, $curr_ver, '>' ) ) {
			// update the version
			update_option( $this->api_manager_license_version_name, $WCMp_Vendor_Frontend_Report->version );
		}
...
function WCMP_Vendor_Frontend_Report_LICENSE() {
  return WCMP_Vendor_Frontend_Report_License::instance();
}