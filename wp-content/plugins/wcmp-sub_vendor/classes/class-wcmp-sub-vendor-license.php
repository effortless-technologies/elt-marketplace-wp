<?php
if (get_option('wcmp_sub_vendor_license_activated') != 'Activated') {
    add_action('admin_notices', 'WCMP_Sub_Vendor_License::license_inactive_notice');
}

class WCMP_Sub_Vendor_License {

    /**
     * Self Upgrade Values
     */
    // Base URL to the remote upgrade API server
    public $upgrade_url = WCMP_SUB_VENDOR_PLUGIN_SERVER_URL; // URL to access the Update API Manager.

    /**
     * @var string
     * This version is saved after an upgrade to compare this db version to $version
     */
    public $api_manager_license_version_name = 'wcmp-sub-vendor_license_version';
    ...
    public $license_update_check = 'wcmp_sub_vendor_update_check';
    ...
    public function __construct() {
        ...
        if (is_admin()) {
             ...
             */
            $this->license_software_product_id = $WCMP_Sub_Vendor->token;

            /**
             * Set all data defaults here
             */
            $this->license_data_key = 'wcmp_' . str_replace('-', '_', esc_attr($WCMP_Sub_Vendor->token)) . '_license_settings_name';
            $this->license_api_key = 'api_key';
            $this->license_activation_email = 'activation_email';
            $this->license_product_id_key = 'wcmp_sub_vendor_license_product_id';
            $this->license_instance_key = 'wcmp_sub_vendor_license_instance';
            $this->license_deactivate_checkbox_key = 'wcmp_sub_vendor_license_deactivate_checkbox';
            $this->license_activated_key = 'wcmp_sub_vendor_license_activated';
            ...
            $this->license_plugin_name = 'wcmp-sub_vendor/sub_vendor.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
            $this->license_product_id = get_option($this->license_product_id_key); // Software Title
            $this->license_renew_license_url = WCMP_SUB_VENDOR_PLUGIN_SERVER_URL . '/my-account'; // URL to renew a license
            ...
            $this->license_software_version = $WCMP_Sub_Vendor->version; // The software version
            ...
            $this->api_manager_license_key = new WCMP_Sub_Vendor_Key_Api();
    ...
    public function add_menu() {
        global $WCMp,$WCMP_Sub_Vendor,$GLOBALS;
        if (empty($GLOBALS['admin_page_hooks']['wcmp_licenses'])) {
            if (!class_exists('WCMp_License')) {
                require_once ($WCMP_Sub_Vendor->plugin_path . 'classes/license/admin/class-wcmp-license.php');
                new WCMp_License('wcmp_sub_vendor_license');
            }
        }
        add_filter('wcmp_license_tabs', array(&$this, 'license_new_tab'), 10, 1);
        add_action('settings_page_' . str_replace('-', '_', esc_attr($WCMP_Sub_Vendor->token)) . '_license_tab_init', array(&$this, 'license_tab_init'), 10, 1);
    }
    ...
    function license_tab_init($tab) {
        ...
        new WCMP_Sub_Vendor_Settings_License($tab);
    }

    function load_class($class_name = '') {
        global $WCMP_Sub_Vendor;
        if ('' != $class_name) {
            require_once ($WCMP_Sub_Vendor->plugin_path . '/classes/license/classes/class-' . esc_attr($WCMP_Sub_Vendor->token) . '-' . esc_attr($class_name) . '.php');
        } // End If Statement
    }
...
// End load_class()
    ...
    function load_class_admin($class_name = '') {
        global $WCMP_Sub_Vendor;
        if ('' != $class_name) {
            require_once ($WCMP_Sub_Vendor->plugin_path . '/classes/license/admin/class-' . esc_attr($WCMP_Sub_Vendor->token) . '-' . esc_attr($class_name) . '.php');
        } // End If Statement
    }
...
// End load_class()
    ...
    public function activation() {
        ...
        $WCMP_Sub_Vendor_API_Manager_Password = new WCMP_Sub_Vendor_API_Manager_Password();
        $instance = $WCMP_Sub_Vendor_API_Manager_Password->generate_password(12, false);
        ...
        if (version_compare($WCMP_Sub_Vendor->version, $curr_ver, '>')) {
            // update the version
            update_option($this->api_manager_license_version_name, $WCMP_Sub_Vendor->version);
        }
...
function WCMP_Sub_Vendor_LICENSE() {
    return WCMP_Sub_Vendor_License::instance();
}
