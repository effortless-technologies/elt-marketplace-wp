...
class WCMp_Vendor_Shop_Seo_Frontend {
    ...
    public function __construct() {
        ...
        if (isset($WCMp_Vendor_Shop_SEO->option_settings['vendor_seo_enabled']) && $WCMp_Vendor_Shop_SEO->option_settings['vendor_seo_enabled'] == 'Enable') {
            add_action('wcmp_after_shop_front', array($this, 'google_analytics_field'), 10, 1);
    ...
    public function google_analytics_field() {
        ...
        echo '<h3>'.__('SEO Settings', $WCMp_Vendor_Shop_SEO->text_domain).'</h3>';
        echo '</div>';
        echo '<div class="panel-body panel-content-padding form-horizontal">';
        $WCMp->wcmp_frontend_fields->wcmp_generate_form_field($this->get_seo_user_field($current_user->ID), array('in_table' => 0));
        echo '</div>';
        echo '</div>';
    }
