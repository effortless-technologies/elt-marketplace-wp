...
class WCMp_License {
    ...
    public function __construct($tab) {
        ...
        add_menu_page(
                __('WCMp License', 'wcmp-vendor_vacation'), __('WCMp License', 'wcmp-vendor_vacation'), 'manage_woocommerce', 'wcmp-license', array($this, 'create_wcmp_license_settings'), $WCMp->plugin_url . 'assets/images/dualcube.png'
        );
    }
    ...
    function wcmp_license_tabs($current = '') {
        ...
        foreach ($this->get_wcmp_license_tabs() as $tab => $name) :
            if ($tab == $current) :
                printf(__("<h2>%s License</h2>", 'wcmp-vendor_vacation'), $name);
            endif;
        endforeach;
        ...
        foreach ($this->get_wcmp_license_tabs() as $tabd => $named) :
            if ($tabd == $current && !empty($tab_desc[$tabd])) :
                printf(__("<h4 style='border-bottom: 1px solid rgb(215, 211, 211);padding-bottom: 21px;'>%s</h4>", 'wcmp-vendor-=_vacation'), $tab_desc[$tabd]);
            endif;
        endforeach;
    }
    ...
    public function create_wcmp_license_settings() {
        ...
        <?php
        $tab = ( isset($_GET['tab']) ? $_GET['tab'] : 'wcmp_vendor_vacation_license' );
