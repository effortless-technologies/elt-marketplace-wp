...
jQuery(document).ready(function($) {
	...
	var pro_types = [ "simple", "variable", "grouped", "external" ];
	if($('#product_type').length > 0) {
		$('#product_type').change(function() {
			...
			if(product_type == 'simple') $('.frontend_product_manager_product_types > .description').removeClass('pro_ele_hide');
  ...
  function resetMultiInputIndex(multi_input_holder) {
  	var holder_id = multi_input_holder.attr('id');
		...
		multi_input_holder.find('.multi_input_block').each(function() {
			$(this).children('.dc-wp-fields-uploader,.multi_input_block_element:not(.multi_input_holder)').each(function() {
				var ele = $(this);
				var ele_name = ele.data('name');
				if(ele.hasClass('dc-wp-fields-uploader')) {
					var uploadEle = ele;
  ...
  function setNestedMultiInputIndex(nested_multi_input, holder_id, holder_name, multi_input_blockCount) {
		...
		nested_multi_input.children('.multi_input_block').children('.dc-wp-fields-uploader,.multi_input_block_element:not(.multi_input_holder)').each(function() {
		  var ele = $(this);
		  var ele_name = ele.data('name');
		  if(ele.hasClass('dc-wp-fields-uploader')) {
				var uploadEle = ele;
