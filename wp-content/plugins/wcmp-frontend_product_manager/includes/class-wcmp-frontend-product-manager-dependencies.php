...
class WCMp_Frontend_Product_Manager_Dependencies {
    ...
    static function fpm_environment_check() {
        global $WCMp;
        $environment = array(
            'pass' => true,
            'is_wcmp_pass' => true,
            'is_import_export_pass' => true,
            'is_pts_pass' => true
        );
        if (!version_compare($WCMp->version, '3.0.0', '>=')) {
            $environment['is_wcmp_pass'] = false;
        }
        if(in_array('wcmp-product_import_export_bundle/wcmp_product_import_export_bundle.php', self::$active_plugins) || array_key_exists('wcmp-product_import_export_bundle/wcmp_product_import_export_bundle.php', self::$active_plugins)){
            $environment['is_import_export_pass'] = false;
        }
        if(in_array('wcmp_fpm/wcmp_pts.php', self::$active_plugins) || array_key_exists('wcmp_fpm/wcmp_pts.php', self::$active_plugins)){
            $environment['is_import_export_pass'] = false;
        }
        if(!$environment['is_wcmp_pass'] || !$environment['is_import_export_pass'] || !$environment['is_import_export_pass']){
