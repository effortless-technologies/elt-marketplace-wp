...
if ( get_option( 'wcmp_product_import_export_bundle_license_activated' ) != 'Activated' ) {
  add_action( 'admin_notices', 'WCMp_Product_Import_Export_Bundle_License::license_inactive_notice' );
}

class WCMp_Product_Import_Export_Bundle_License {
	
	/**
	 * Self Upgrade Values
	 */
	// Base URL to the remote upgrade API server
	public $upgrade_url = WCMP_PRODUCT_IMPORT_EXPORT_BUNDLE_PLUGIN_SERVER_URL; // URL to access the Update API Manager.

	/**
	 * @var string
	 * This version is saved after an upgrade to compare this db version to $version
	 */
	public $api_manager_license_version_name = 'wcmp-product-import-export-bundle_license_version';
	...
	public $license_update_check = 'wcmp_product_import_export_bundle_update_check';
    ...
    global $WCMp_Product_Import_Export_Bundle;
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->license_software_product_id = $WCMp_Product_Import_Export_Bundle->token;

			/**
			 * Set all data defaults here
			 */
			$this->license_data_key 				= 'wcmp_' . str_replace('-', '_', esc_attr($WCMp_Product_Import_Export_Bundle->token)) . '_license_settings_name';
			$this->license_api_key 					= 'api_key';
			$this->license_activation_email 		= 'activation_email';
			$this->license_product_id_key 			= 'wcmp_product_import_export_bundle_license_product_id';
			$this->license_instance_key 			= 'wcmp_product_import_export_bundle_license_instance';
			$this->license_deactivate_checkbox_key 	= 'wcmp_product_import_export_bundle_license_deactivate_checkbox';
			$this->license_activated_key 			= 'wcmp_product_import_export_bundle_license_activated';
			...
			$this->license_plugin_name 			= 'wcmp-product_import_export_bundle/wcmp_product_import_export_bundle.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
			$this->license_product_id 			= get_option( $this->license_product_id_key ); // Software Title
			$this->license_renew_license_url 	= WCMP_PRODUCT_IMPORT_EXPORT_BUNDLE_PLUGIN_SERVER_URL.'/my-account'; // URL to renew a license
			...
			$this->license_software_version 	= $WCMp_Product_Import_Export_Bundle->version; // The software version
      ...
      $this->api_manager_license_key = new WCMp_Product_Import_Export_Bundle_Key_Api();
      ...
      $this->load_class('update');
			...
			if ( ! empty( $options ) && $options !== false ) {
				...
				new WCMp_Product_Import_Export_Bundle_API_Manager_Update_API_Check(
					$this->upgrade_url,
					...
					'wcmp-product-import-export-bundle'
				);
	...
	public function add_menu() {
	  global $WCMp, $WCMp_Product_Import_Export_Bundle, $GLOBALS;
	  
	  if ( empty ( $GLOBALS['admin_page_hooks']['wcmp_licenses'] ) )  {
	  	if(!class_exists('WCMp_License')) {
	  		require_once ($WCMp_Product_Import_Export_Bundle->plugin_path . 'classes/license/admin/class-wcmp-license.php');
	  		new WCMp_License('wcmp_product_import_export_bundle_license');
	  	}
		}
		
	  add_filter('wcmp_license_tabs', array(&$this, 'license_new_tab'), 10, 1);
	  add_action( 'settings_page_' . str_replace('-', '_', esc_attr($WCMp_Product_Import_Export_Bundle->token)) . '_license_tab_init', array(&$this, 'license_tab_init'), 10, 1);
	}
	
	function license_new_tab($tabs) {
	  global $WCMp_Product_Import_Export_Bundle;
	  $tabs[str_replace('-', '_', esc_attr($WCMp_Product_Import_Export_Bundle->token)) . '_license'] = __('Import / Export Bundle', 'wcmp-product-import-export-bundle');
    return $tabs;
	}
	
	function license_tab_init($tab) {
    $this->load_class_admin("settings-license");
    new WCMp_Product_Import_Export_Bundle_Settings_License($tab);
  }
	
	function load_class($class_name = '') {
	  global $WCMp_Product_Import_Export_Bundle;
		if ('' != $class_name) {
			require_once ($WCMp_Product_Import_Export_Bundle->plugin_path . '/classes/license/classes/class-' . esc_attr($WCMp_Product_Import_Export_Bundle->token) . '-' . esc_attr($class_name) . '.php');
		} // End If Statement
	}// End load_class()
	...
	function load_class_admin($class_name = '') {
	  global $WCMp_Product_Import_Export_Bundle;
		if ('' != $class_name) {
			require_once ($WCMp_Product_Import_Export_Bundle->plugin_path . '/classes/license/admin/class-' . esc_attr($WCMp_Product_Import_Export_Bundle->token) . '-' . esc_attr($class_name) . '.php');
		} // End If Statement
	}// End load_class()
	...
	public function activation() {
		global $wpdb, $WCMp_Product_Import_Export_Bundle;
		...
		$WCMp_Product_Import_Export_Bundle_API_Manager_Password = new WCMp_Product_Import_Export_Bundle_API_Manager_Password();
		$instance = $WCMp_Product_Import_Export_Bundle_API_Manager_Password->generate_password( 12, false );
		...
		if ( version_compare( $this->license_software_version, $curr_ver, '>' ) ) {
			// update the version
			update_option( $this->api_manager_license_version_name, $WCMp_Product_Import_Export_Bundle->version );
		}
	...
	public function wcmp_plugin_tracker($status, $api_key = '', $api_email = '') {
	  ...
	  $api_url = add_query_arg( 'wc-api', 'wcmp-plugin-tracker', $this->upgrade_url );
	...
	public static function license_inactive_notice() {
	  ...
	  ?>
		...
		<div id="message" class="error settings-error notice is-dismissible">
			<p><?php printf( __( 'The Product Import Export Bundle License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'wcmp-product-import-export-bundle' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wcmp-license&tab=' . str_replace('-', '_', esc_attr($WCMp_Product_Import_Export_Bundle->token)) . '_license' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
...
function WCMp_Product_Import_Export_Bundle_LICENSE() {
  return WCMp_Product_Import_Export_Bundle_License::instance();
}
