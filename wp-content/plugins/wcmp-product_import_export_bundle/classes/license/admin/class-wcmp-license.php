...
class WCMp_License {
	...
	public function __construct($tab) {
		...
		add_menu_page(
				__('WCMp License', 'wcmp-product-import-export-bundle'), 
				__('WCMp License', 'wcmp-product-import-export-bundle'), 
				'manage_woocommerce', 
				'wcmp-license', 
  ...
  function wcmp_license_tabs( $current = '' ) {
    ...
    foreach( $this->get_wcmp_license_tabs() as $tab => $name ) :
      if ( $tab == $current ) :
        printf( __( "<h2>%s License</h2>", 'wcmp-product-import-export-bundle') , $name);
      endif;
    endforeach;
    ...
    foreach( $this->get_wcmp_license_tabs() as $tabd => $named ) :
      if ( $tabd == $current && !empty($tab_desc[$tabd]) ) :
        printf( __( "<h4 style='border-bottom: 1px solid rgb(215, 211, 211);padding-bottom: 21px;'>%s</h4>", 'wcmp-product-import-export-bundle') , $tab_desc[$tabd]);
      endif;
    endforeach;
  }
  ...
  public function create_wcmp_license_settings() {
    ...
    <div class="wrap">
      ...
      <?php
      $tab = ( isset( $_GET['tab'] ) ? $_GET['tab'] : 'wcmp_product_import_export_bundle_license' );
