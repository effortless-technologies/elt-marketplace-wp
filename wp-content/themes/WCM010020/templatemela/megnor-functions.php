...
{
	add_option("tmoption_logo_image", get_stylesheet_directory_uri()."/images/logo.png"); // set logo image	
	add_option("tmoption_logo_image_alt",'Branchy'); // set logo image alt
	...
	add_option("tmoption_bodyfont_color","696969"); // body font color
	add_option("tmoption_bodyfont",'Istok+Web'); // Open Sans	
	add_option("tmoption_bodyfont_other",'sans-serif'); // Arial
	add_option("tmoption_button_color","2FB991"); // button color
	...
	add_option("tmoption_topbar_text_color","555555"); // topbar_text_color
	add_option("tmoption_topbar_link_color","555555"); // topbar_link_color
	add_option("tmoption_topbar_link_hover_color","2FB991"); // topbar_link_hover_color
	add_option("tmoption_contact_panel","yes");
	add_option("tmoption_topbar_phone","Toll Free :0123-456-789");
	add_option("tmoption_Contact_Text","Call Us Now");
	add_option("tmoption_custom_banner","yes"); // Show Custom Banner
	add_option("tmoption_custom_text1","UP TO 5% REWARD"); // Custom Text 1
	add_option("tmoption_custom_text1_desc","At vero eos et accusamus et iusto odio dignis"); // Custom Text 1 desc
	add_option("tmoption_custom_text2","DELIVERY SCHEDULE"); // Custom Text 2
	add_option("tmoption_custom_text2_desc","At vero eos et accusamus et iusto odio dignis"); // Custom Text 2 desc
	add_option("tmoption_custom_text3","SAME DAY SHIPPING"); // Custom Text 3	
	...
	add_option("tmoption_headerbkg_color","EFEFEF");// header background Color	
	add_option("tmoption_menubg_color","999999"); // Menu Background Color
	
	/*  Content Settings  */
	add_option("tmoption_h1font",'Istok+Web'); // h1 family google font
	add_option("tmoption_h1font_other",'Arial'); // h1 family specified font
	add_option("tmoption_h1color",'191919'); // h1 family font color	 
	add_option("tmoption_h2font",'Istok+Web'); // h2 family google font
	add_option("tmoption_h2font_other",'Arial'); // h2 family specified font
	add_option("tmoption_h2color",'191919'); // h2 family font color	
	add_option("tmoption_h3font",'Istok+Web'); // h3 family google font
	add_option("tmoption_h3font_other",'Arial'); // h3 family specified font
	add_option("tmoption_h3color",'191919'); // h3 family font color	
	add_option("tmoption_h4font",'Istok+Web'); // h4 family google font
	add_option("tmoption_h4font_other",'Arial'); // h4 family specified font
	add_option("tmoption_h4color",'606060'); // h4 family font color	
	add_option("tmoption_h5font",'Istok+Web'); // h5 family google font
	add_option("tmoption_h5font_other",'Arial'); // h5 family specified font 
	add_option("tmoption_h5color",'606060'); // h5 family font color	
	add_option("tmoption_h6font",'Istok+Web'); // h6 family google font
	add_option("tmoption_h6font_other",'Arial'); // h6 family specified font 
	add_option("tmoption_h6color",'606060'); // h6 family font color	
	add_option("tmoption_link_color","555555"); // link color
	add_option("tmoption_hoverlink_color","2FB991"); // link hovre color
	
	/*  Footer Settings  */	
	add_option("tmoption_footerbkg_color","EFEFEF"); // footer background color
	add_option("tmoption_footerlink_color","777777"); // footer link text color
	add_option("tmoption_footerhoverlink_color","2FB991"); // footer link hover text color	
	add_option("tmoption_footer_slog",'Templatemela.'); // copyright statement : Theme By templatemela
	add_option("tmoption_footer_link",'https://www.google.co.in/'); // copyright link : Theme By templatemela

	/* Shop page settings */
	add_option("tmoption_shop_items","16");	
	add_option("tmoption_shop_per_items","4");	
...
function templatemela_excerpt_desc($limit) {
...
function templatemela_product_desc($limit) {
$excerpt = explode(' ', templatemela_strip_images(strip_tags(get_the_excerpt())), $limit);
      if (count($excerpt)>=$limit) {
        array_pop($excerpt);
                $excerpt = implode(" ",$excerpt).'&nbsp;<a class="read-more" href="'.get_permalink().'">Read More</a>';
      } else {
        $excerpt = implode(" ",$excerpt);
      } 
      $excerpt = preg_replace('`\[[^\]]*\]`','',$excerpt);
...
get_template_part('templatemela/custom-post/slider');
...
function templatemela_load_fonts() {
	...
	wp_enqueue_style( 'opensans-user', "$protocol://fonts.googleapis.com/css?family=Open+Sans" );
	wp_enqueue_style( 'IstokWeb-user', "$protocol://fonts.googleapis.com/css?family=Istok+Web:400,700,400italic,700italic" );	
	wp_enqueue_style( 'ArchivoNarrow-user', "$protocol://fonts.googleapis.com/css?family=Archivo+Narrow:300,400,500,600,700" );
}
endif;
...
function templatemela_load_scripts() {	
	...
	wp_enqueue_script( 'branchy_jquery_waypoints', get_template_directory_uri() . '/js/megnor/waypoints.min.js', array(), '', false);
	...
	wp_enqueue_script( 'jquery_easing', get_template_directory_uri() . '/js/megnor/jquery.easing.1.3.js', array(), '', true);	
	wp_enqueue_script( 'jqueryeislideshow', get_template_directory_uri() . '/js/megnor/jquery.eislideshow.js', array(), '', true);	
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
  <div id="comment-<?php comment_ID(); ?>" class="comment-body">
    ...
    <div class="author-content">
      <h6><?php echo attr($comment->comment_author); ?></h6>
...
function templatemela_get_topbar_banner() {	
	...
	$output .= '<ul class="top-banner-container">';
		...
		$output .= '<li class="content1 content"><div class="content-inner"><i class="fa fa-usd"></i><div class="cms"><div class="cms-title">'.$tmoption_custom_text1.'</div><div class="cms-desc">'.$tmoption_custom_text1_desc.'</div></div></div></li>';	
		endif;
		if (!empty($tmoption_custom_text2)):
		$output .= '<li class="content2 content"><div class="content-inner"><i class="fa fa-calendar"></i><div class="cms"><div class="cms-title">'.$tmoption_custom_text2.'</div> <div class="cms-desc">'.$tmoption_custom_text2_desc.'</div></div></div></li>';	
		endif;
		if (!empty($tmoption_custom_text3)):
		$output .= '<li class="content3 content"><div class="content-inner"><i class="fa fa-gift"></i><div class="cms"><div class="cms-title">'.$tmoption_custom_text3.'</div> <div class="cms-desc">'.$tmoption_custom_text3_desc.'</div></div></div></li>';				
		endif;	
	$output .= '</ul>';	
	$output .= '</div>';
	echo $output;
...
add_image_size( 'blog-posts-list', 350, 300, true );
...
function templatemela_related_products_args( $args ) {
  	$no = get_option("tmoption_related_items");	
	$args['posts_per_page'] = $no; 
	$args['columns'] = get_option('tmoption_shop_items');
	return $args;
}
add_filter( 'woocommerce_output_related_products_args', 'templatemela_related_products_args' );

/* Upsell Product settings */
	function templatemela_output_upsells() {
		$no1 = get_option("tmoption_upsells_items");	
	   	woocommerce_upsell_display( $no1, get_option('tmoption_shop_items')); 
	}
remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15 );
add_action( 'woocommerce_after_single_product_summary', 'templatemela_output_upsells', 15 );
	

/* crosssell Product settings */
	function templatemela_output_crosssell() {
		$no1 = get_option("tmoption_crosssell_items");	
	   	woocommerce_cross_sell_display( $no1, get_option('tmoption_shop_items')); 
	}
remove_action( 'woocommerce_cart_collaterals', 'woocommerce_cross_sell_display', 10 );
add_action( 'woocommerce_after_cart', 'templatemela_output_crosssell', 10 );


// Display products per row shop page
add_filter( 'loop_shop_columns', 'templatemela_product_per_row' );   	
function templatemela_product_per_row() {
	$no1 = get_option("tmoption_shop_per_items");
	return $no1;
}

/*
remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15 );
add_action( 'woocommerce_after_single_product_summary', 'templatemela_woocommerce_output_upsells', 15 );*/
...
function templatemela_override_woocommerce_widgets() {
  // Ensure our parent class exists to avoid fatal error (thanks Wilgert!)
 
  if ( class_exists( 'WC_Widget_Product_Categories' ) ){
    unregister_widget( 'WC_Widget_Product_Categories') ;
    include_once 'widgets/wc-widget-product-categories.php';
    register_widget( 'Custom_WC_Widget_Product_Categories' );
  }
}
add_action( 'widgets_init', 'templatemela_override_woocommerce_widgets', 15) ; 
...
function tm_googleMap($latlong, $text, $pin, $height, $id, $class) {
		...
		return '<div class="google_map_outer"><div id="map'.$id.'" class="google-map '.$class.'" '.$height.'></div></script></div>
...
if( ! function_exists( 'branchy_add_to_wishlist_in_product' ) ){
    function branchy_add_to_wishlist_in_product(){
        echo do_shortcode( "[yith_wcwl_add_to_wishlist]" );
    }
}
add_action( 'woocommerce_before_shop_loop_item_title', 'branchy_add_to_wishlist_in_product', 11 );
endif;

//dequeue css from plugins
add_action('wp_print_styles', 'branchy_dequeue_css_from_plugins', 100);
function branchy_dequeue_css_from_plugins()  {
